...
void main() {
  ...
  group('flattenEntryTree', () {
    List<EntryYearGroup> twoDaysInTwoYears() => buildEntryTree([
      _entry(
        id: '1',
        startAt: DateTime(2026, 8, 18, 9),
        endAt: DateTime(2026, 8, 18, 10),
      ...
      _entry(
        id: '2',
        startAt: DateTime(2025, 3, 4, 9),
        endAt: DateTime(2025, 3, 4, 10),
      ),
    ], tiers: const []);

    test('shows one row per year when nothing is expanded', () {
      final rows = flattenEntryTree(twoDaysInTwoYears(), const {});
    ...
    test('an expanded year adds exactly its months', () {
      final rows = flattenEntryTree(twoDaysInTwoYears(), {yearTreeKey(2026)});
    ...
    test('the entries row appears only for an expanded day', () {
      final tree = twoDaysInTwoYears();
      final downToTheDayRow = {
        yearTreeKey(2026),
        monthTreeKey(2026, 8),
        weekTreeKey(DateTime(2026, 8, 17), 2026, 8),
      };
    ...
    test('ignores keys for nodes that no longer exist', () {
      final rows = flattenEntryTree(twoDaysInTwoYears(), {
        yearTreeKey(1999),
