...
> **ALWAYS**: Keep instructions cross-AI understandable (Claude, GPT, Gemini, and other coding models)
...
> **ALWAYS**: Validate and sanitize ALL inputs (user input, API responses, DB results, file contents) — type, format, bounds, size limits
> **ALWAYS**: Check null/undefined/empty and collection bounds before access; handle boundary cases (empty, zero, negative, max) and race conditions
> **ALWAYS**: Handle all error cases explicitly (try-catch, network failures, timeouts) with fail-safe defaults
> **ALWAYS**: Log errors with context (timestamp, user, operation, stack trace); meaningful messages in logs only
> **ALWAYS**: Use parameterized queries, escape output in templates, hash passwords (bcrypt/Argon2), HTTPS, rate limiting, file-upload validation (type, size, content)
> **ALWAYS**: Use environment variables for secrets (never hardcode)
>
> **NEVER**: Trust external input — no exception for "trusted" sources or client-side validation
> **NEVER**: Use eval/exec with user input, weak hashes (MD5/SHA1) for passwords, or plaintext passwords
> **NEVER**: Use empty catch blocks or let unhandled exceptions crash the application
> **NEVER**: Expose internal error details or sensitive data to end users, error messages, or logs
...
>
> 1. **CHECK** the ai-iap code library index (`C:/Users/mordi/.claude/plugins/cache/mordilion/ai-iap/1.4.0/lib/code-library/INDEX.md`) for core patterns overview
